...
public class MysqlDatabase implements JdbcDatabase {
    ...
    public void executeTableDDL(Connection connection) throws SQLException {
        try (Statement statement = connection.createStatement()) {
            statement.execute("create table if not exists `id_gen_server_lock` (" +
                              "`id` int not null," +
            ...
            statement.execute("create table if not exists `id_gen_config` (" +
    ...
    @Override
    public Locker queryExpired(Connection connection) throws SQLException {
        String sql = "select id,lock_key from id_gen_server_lock where id=(select min(id) " +
    ...
    public int queryNextId(Connection connection) throws SQLException {
        String sql = "select ifNull(min(l.id+1),0) as id from id_gen_server_lock l " +
                     "left join id_gen_server_lock r on l.id = r.id-1 where r.id is null";
        try (Statement statement = connection.createStatement();
    ...
    public int lock(Connection connection, int lockId, String oldKey, String newKey, int expirySeconds) throws SQLException {
        String sql = "update id_gen_server_lock " +
    ...
    public boolean newLock(Connection connection, int id, int expirySeconds, String key) throws SQLException {
        String sql = "insert into `id_gen_server_lock` (`id`,`expiry_time`,`lock_key`) " +
    ...
    public long queryAwaitTime(Connection connection, int maxId) throws SQLException {
        String sql = "select timestampdiff(microsecond,now(),min(expiry_time))/1000 as wait_time " +
                     "from id_gen_server_lock where id between 0 and ?";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
    ...
    public long queryTimeOffsetConfig(Connection connection) {
        String sql = "select config from id_gen_config where id='time_offset'";
        try {
            try (Statement statement = connection.createStatement();
                 ResultSet resultSet = statement.executeQuery(sql)) {
                if (resultSet.next()) {
                    ...
                    String insertSql = "insert into id_gen_config (id,config) " +
